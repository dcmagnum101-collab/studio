      ...
      <SheetContent className="sm:max-w-xl w-full p-0 flex flex-col">
        <SheetHeader className="p-6 bg-slate-50/50 border-b">
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                ...
                {contact.listing_status === 'FOR_SALE' && (
                  <Badge className="bg-green-500 text-[10px]">ACTIVE MLS</Badge>
                )}
              </div>
          ...
          <div className="flex gap-2 mt-4">
            ...
            <Button size="sm" variant="outline" className="flex-1 gap-2 border-slate-200"><Building2 className="h-4 w-4" /> Trulia Intel</Button>
          </div>
        </SheetHeader>
          ...
          <div className="p-8 space-y-10">
            <section className="space-y-4">
              <div className="flex items-center gap-2 text-sm font-bold text-slate-500 uppercase tracking-widest">
                <Building2 className="h-4 w-4" /> Trulia Market Status
              </div>
              <div className="bg-primary/5 p-6 rounded-2xl border border-primary/10">
                <div className="flex justify-between items-center mb-4">
                  <div className="space-y-1">
                    <div className="text-[10px] text-muted-foreground uppercase">MLS Status</div>
                    <div className="text-sm font-black text-primary uppercase">{contact.listing_status || 'Off Market'}</div>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-3 bg-white rounded-xl border border-slate-100">
                    <span className="text-[9px] text-muted-foreground uppercase block">Days on Market</span>
                    <span className="text-xs font-bold">{contact.days_on_market || '0'} days</span>
                  </div>
                  <div className="p-3 bg-white rounded-xl border border-slate-100">
                    <span className="text-[9px] text-muted-foreground uppercase block">Motivation Signal</span>
                    <span className="text-xs font-bold text-orange-600">{contact.listing_status === 'FOR_SALE' ? 'Active Seller' : 'Nurture'}</span>
                  </div>
                </div>
              </div>
            </section>
            ...
            <section className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-6 bg-slate-50 p-6 rounded-2xl border border-slate-100">
                ...
                <div className="space-y-1">
                  ...
                  <div className="text-sm font-semibold capitalize">{contact.property_type?.replace('_', ' ') || 'Single Family'}</div>
                </div>
                <div className="space-y-1">
                  <div className="text-[10px] text-muted-foreground uppercase">Equity Status (LTV)</div>
                  <div className="space-y-2">
                    <Progress value={contact.loan_to_value || 40} className="h-2" />
                    <div className="text-xs font-bold text-primary">{contact.loan_to_value || 40}% LTV — High Equity</div>
                  </div>
                </div>
              </div>
            </section>
            ...
            <section className="space-y-4 pb-12">
              ...
              <div className="space-y-6">
                {contact.activityLogs?.length > 0 ? (
                  contact.activityLogs.map((log: any) => (
