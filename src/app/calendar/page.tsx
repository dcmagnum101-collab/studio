...
export default function CalendarPage() {
  ...
  const getDayEvents = (day: Date) => {
    const appointments = MOCK_APPOINTMENTS.filter(appt => isSameDay(new Date(appt.date), day))
    const tasks = MOCK_TASKS.filter(task => isSameDay(new Date(task.due_date), day))
              ...
              <div className="flex-1 border rounded-2xl bg-white shadow-sm overflow-hidden flex flex-col">
                ...
                <div className="flex-1 grid grid-cols-7 grid-rows-5 overflow-y-auto">
                  {calendarDays.map((day, idx) => {
                    const { appointments, tasks } = getDayEvents(day)
                    ...
                    return (
                      ...
                      >
                        <div className="flex justify-between items-center mb-1">
                          <span className={`text-xs font-bold h-6 w-6 flex items-center justify-center rounded-full ${isToday ? 'bg-primary text-white' : ''}`}>
                            {format(day, 'd')}
                          </span>
                        </div>
                        ...
                        <div className="flex-1 flex flex-col gap-1 overflow-hidden">
                          {appointments.map(appt => (
                            <div key={appt.id} className="bg-blue-100 text-blue-700 text-[10px] p-1 rounded-md border border-blue-200 truncate font-medium flex items-center gap-1">
                              <div className="h-1.5 w-1.5 rounded-full bg-blue-500" />
                              {appt.title}
                            </div>
                          ))}
                          {tasks.map(task => (
                            <div key={task.id} className="bg-yellow-100 text-yellow-700 text-[10px] p-1 rounded-md border border-yellow-200 truncate font-medium flex items-center gap-1">
            ...
            <aside className="w-80 border-l bg-white p-6 space-y-8 overflow-y-auto">
              <section className="space-y-4">
                ...
                <div className="space-y-3">
                  {MOCK_APPOINTMENTS.filter(a => isSameDay(new Date(a.date), new Date())).length > 0 ? (
                    MOCK_APPOINTMENTS.filter(a => isSameDay(new Date(a.date), new Date())).map(appt => (
                        ...
                        <CardContent className="p-4 space-y-2">
                          ...
                          <div className="space-y-1">
                            <div className="flex items-center gap-2 text-[10px] text-blue-700">
                              <Clock className="h-3 w-3" /> {format(new Date(appt.date), 'h:mm a')} ({appt.duration_minutes} min)
                            </div>
                            <div className="flex items-center gap-2 text-[10px] text-blue-700">
              ...
              <section className="space-y-4">
                ...
                <div className="space-y-2">
                  {MOCK_TASKS.filter(t => t.priority === 'urgent' && t.status !== 'completed').map(task => (
                    <div key={task.id} className="p-3 bg-red-50 border border-red-100 rounded-xl flex items-start gap-3">
